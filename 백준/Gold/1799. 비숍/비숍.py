# backtracking + branch & bound 알고리즘
import sys
...
# 우하향(right down) 대각선
rd = {}
# 중앙 기준으로 쁠마. (y,x) -> rd[x-y], 왼쪽 아래가 -(n-1), 오른쪽 위가 (n-1)
for i in range(-n+1, n):
    rd[i] = 0

# 현재 대각선(우상향) 위치에서부터 끝 대각선까지 뽑힐 가능성이 있는 애들의 개수 반환
...
# diag은 우상향 대각선의 번호. 첫 우상향 대각선이 0, 마지막이 2*n-2
def f(diag, cnt):
    global ans
    if diag == 2*n:
        ans = max(ans, cnt)
    ...
    if ub + cnt <= ans:     # 현재 cnt값과 앞으로 최대한 얻을 수 있는 값을 더해도 기존 ans보다 작으면
        return              # 빠져나온다
    ...
    for y in range(diag+1): # 현재 대각선에서 가능한 y,x 조합 찾기
        ...
        if in_range(y, x) and board[y][x] and rd[x-y] == 0:
            ...
            f(diag+1, cnt+1) # (y,x) 좌표에 비숍 넣었으므로 cnt+1하고 다음 우상향 대각선으로 감(diag+1)
            rd[x-y] = 0
